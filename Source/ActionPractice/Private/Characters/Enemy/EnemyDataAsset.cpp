...
#if ENABLE_DEBUG_LOG
	DEFINE_LOG_CATEGORY_STATIC(LogEnemyDataAsset, Log, All);
#define DEBUG_LOG(Format, ...) UE_LOG(LogEnemyDataAsset, Warning, Format, ##__VA_ARGS__)
#else
...
void UEnemyDataAsset::PreloadAllMontages()
{
	...
	if (!AssetId.IsValid())
	{
		...
		DEBUG_LOG(TEXT("PreloadAllMontages: invalid PrimaryAssetId — check PrimaryAssetTypesToScan(EnemyData) settings"));
		return;
	}

	bPreloadRequested = true;

	//Combat 번들(공격 콤보 + 사망 + 히트리액션 + 그로기 몽타주 전부)을 AssetManager 번들 단위로 로드
	...
	//C그룹 어빌리티(Death/Groggy/HitReaction)가 같은 몽타주를 중복 요청해도 참조 카운팅으로 안전하다
	BundleHandle = UAssetManager::Get().ChangeBundleStateForPrimaryAssets(
		...
		{
			DEBUG_LOG(TEXT("EnemyDataAsset Combat bundle load complete"));
		}),
